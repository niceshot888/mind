...
and b1.id not in (select min(id) from {0} GROUP BY SECURITYCODE,REPORTDATE HAVING count(*) > 1)";
        ...
        public static void OperateMajorBusinessScopeData(string ts_code, em_major_business_scope_item jo, string type)
        {
            try
            {
                var majorBizScope = GetMajorBusinessScopeData(ts_code, jo.rq, type, jo.zygc);
                if (majorBizScope != null)
                {
                    //修改数据
                    Logger.Info(string.Format("update data: tscode={0};date={1}", ts_code, jo.rq));
                    string sql = "update em_major_business_scope set zygc=@zygc,zysr=@zysr,srbl=@srbl,zycb=@zycb,cbbl=@cbbl,zylr=@zylr,lrbl=@lrbl,mll=@mll,type=@type where id=@id";
                    using (IDbConnection conn = new MySqlConnection(MySqlDbHelper.NICESHOTDB_CONN_STRING))
                    {
                        conn.Execute(sql, new
                        {
                            zygc = jo.zygc,
                            zysr = jo.zysr,
                            srbl = jo.srbl.Replace("--","").Replace("%","").ConvertToDecimal(),
                            zycb = jo.zycb,
                            cbbl = jo.cbbl.Replace("--", "").Replace("%", "").ConvertToDecimal(),
                            zylr = jo.zylr,
                            lrbl = jo.lrbl.Replace("--", "").Replace("%", "").ConvertToDecimal(),
                            mll = jo.mll.Replace("--", "").Replace("%", "").ConvertToDecimal(),
                            type = type,
                            id = majorBizScope.id
                        });
                    }
                ...
                {
                    //添加数据
                    Logger.Info(string.Format("insert data: tscode={0};date={1}", ts_code, jo.rq));

                    string sql = "insert into em_major_business_scope(zygc,zysr,srbl,zycb,cbbl,zylr,lrbl,mll,ts_code,date,type) values (@zygc,@zysr,@srbl,@zycb,@cbbl,@zylr,@lrbl,@mll,@ts_code,@date,@type)";

                    var entity = new em_major_business_scope()
                    {
                        zygc = jo.zygc,
                        zysr = jo.zysr,
                        srbl = jo.srbl.Replace("--", "").Replace("%", "").ConvertToDecimal(),
                        ...
                        type = type,
                        ts_code = ts_code,
                        date = jo.rq.ConvertToDate().GetValueOrDefault(),
                    };
                    using (IDbConnection conn = new MySqlConnection(MySqlDbHelper.NICESHOTDB_CONN_STRING))
                    {
                        conn.Execute(sql, entity);
            ...
            {
                Logger.Error(string.Format("sync a stock major biz scope error: tscode={0};date={1},详细：{2}", ts_code, jo.rq, ex));
            }

        }

        public static em_major_business_scope GetMajorBusinessScopeData(string ts_code, string date, string type, string zygc)
        {
            using (var conn = new MySqlConnection(MySqlDbHelper.NICESHOTDB_CONN_STRING))
            {
                string sql = "select * from em_major_business_scope where ts_code='" + ts_code + "' and date='" + date + "' and type='" + type + "' and zygc='"+zygc+"'";
                return conn.Query<em_major_business_scope>(sql).FirstOrDefault();
            }
        }

        public static void OperateCoreConceptData(string ts_code, em_core_concept_item jo)
        {
            try
            {
                var totalPinyin = PinYinConverterHelper.GetTotalPingYin(jo.BOARD_NAME);
                string pinyin = string.Join(",", totalPinyin.FirstPingYin);
                string fullpinyin = string.Join(",", totalPinyin.TotalPingYin);

                var data = GetCoreConceptData(ts_code, jo.BOARD_NAME);
                if (data != null)
                {
                    //修改数据
                    Logger.Info(string.Format("update data: tscode={0};name={1}", ts_code, jo.BOARD_NAME));
                    string sql = "update em_core_concept set board_name=@board_name,pinyin=@pinyin,fullpinyin=@fullpinyin where id=@id";
                    using (var conn = new MySqlConnection(MySqlDbHelper.NICESHOTDB_CONN_STRING))
                    {
                        conn.Execute(sql, new
                        {
                            board_name = jo.BOARD_NAME,
                            pinyin = pinyin,
                ...
                {
                    //添加数据
                    Logger.Info(string.Format("insert data: tscode={0};name={1}", ts_code, jo.BOARD_NAME));

                    var sql = "insert into em_core_concept(ts_code,board_name,is_precise,pinyin,fullpinyin) values (@ts_code,@board_name,@is_precise,@pinyin,@fullpinyin)";

                    var entity = new em_core_concept()
                    {
                        board_name = jo.BOARD_NAME,
                        ts_code = jo.SECUCODE,
                        is_precise = int.Parse(jo.IS_PRECISE),
                        pinyin = pinyin,
                        fullpinyin = fullpinyin
                    };
                    using (var conn = new MySqlConnection(MySqlDbHelper.NICESHOTDB_CONN_STRING))
            ...
            {
                Logger.Error(string.Format("sync a stock major biz scope error: tscode={0};name={1},详细：{2}", ts_code, jo.BOARD_NAME, ex));
            }

        }

        public static em_core_concept GetCoreConceptData(string ts_code,string board_name)
        {
            using (var conn = new MySqlConnection(MySqlDbHelper.NICESHOTDB_CONN_STRING))
            {
                var sql = "select * from em_core_concept where ts_code='" + ts_code + "' and board_name='" + board_name + "'";
                return conn.Query<em_core_concept>(sql).FirstOrDefault();
            }
        }
