...
namespace NiceShot.Core.DataAccess
{
    public class XQDataAccess
    {
        ...
        public static void OperateMacdData(string securitycode, decimal? dea, decimal? dif, decimal? macd,decimal? cci,decimal? pb)
        {
            try
            {
                var stockInfo = NiceShotDataAccess.GetStockInfo(securitycode, MarketType.A);
                if (stockInfo != null)
                {
                    var sql = "update xq_stock set macd=@macd,dif=@dif,dea=@dea,cci=@cci,pb=@pb where id=@id";
                    ...
                    Logger.Info("update xueqiu stock macd：" + macd);
                }
        ...
        public static List<foreign_debt> GetForeighDebtList()
        {
            using (IDbConnection conn = new MySqlConnection(MySqlDbHelper.NICESHOTDB_CONN_STRING))
            {
                string sql = "select * from foreign_debt where code is not null";

                var datalist = conn.Query<foreign_debt>(sql).ToList();

                return datalist;
            }
        }

        public static foreign_debt GetForeighDebt(string code)
        {
            using (IDbConnection conn = new MySqlConnection(MySqlDbHelper.NICESHOTDB_CONN_STRING))
            {
                string sql = "select * from foreign_debt where code='" + code + "'";

                return conn.Query<foreign_debt>(sql).FirstOrDefault();
            }
        }

        public static void OperateForeighDebt(DateTime date, string securitycode, string title, string text, List<string> p_text_list)
        {
            try
            {
                var entity = GetForeighDebt(securitycode);
                if (entity != null)
                {
                    string sql = "update foreign_debt set guimo=@guimo,jiaogeri=@jiaogeri,daoqiri=@daoqiri,pingji=@pingji,xipiaolv=@xipiaolv,shouyilv=@shouyilv,title=@title,faxingren=@faxingren,text=@text,date=@date where code=@code";

                    var guimo = GetPText(p_text_list, "规模：");
                    var jiaogeri = GetPText(p_text_list, "交割日");
                    var daoqiri = GetPText(p_text_list, "到期日");
                    var pingji = GetPText(p_text_list, "母公司担保人评级,发行人评级,担保人评级,维好协议和股权购买投资承诺协议提供人评级");
                    var xipiaolv = GetPText(p_text_list, "息票率");
                    var shouyilv = GetPText(p_text_list, "收益率");
                    var faxingren = GetPText(p_text_list, "发行人");
            ...
            {
                Logger.Error(string.Format("同步雪球搜索数据失败: tscode={0};详细：{1}", securitycode, ex));
            }
        }

        private static string GetPText(List<string> p_text_list, string title_str)
        {
            var p_text = string.Empty;
            foreach (var text in p_text_list)
            {
                ...
                foreach (var title in title_list)
                {
                    ...
                    {
                        p_text = text.Replace(title + ":", "").Replace(title + "：", "");
                        break;
                    }
                }
            }
