...
namespace NiceShot.DotNetWinFormsClient
{
    public partial class MainForm : Form
    {
        ...
        private void cms_stock_details_Opening(object sender, CancelEventArgs e)
        {
            ...
            ((ToolStripMenuItem)cms.Items["tsmi_otherfunc"]).DropDownItems["tsmi_is_follow"].Text = is_follow == "已关注" ? "取消关注" : "关注";
        ...
        private void tsm_a_normal_Click(object sender, EventArgs e)
        {
            ...
            //this.scp_toppanel.tbxIndustry.Text = "房地产";
            //this.Width = 1400;
            this.CenterToScreen();
        ...
        private void tsm_hk_common_Click(object sender, EventArgs e)
        {
            ...
            //this.scp_toppanel.tbxIndustry.Text = "地产";
            //this.Width = 920;
            this.CenterToScreen();
        ...
        private void tsmi_stock_analysis_Click(object sender, EventArgs e)
        {
            ...
            //StockDetailsForm frmStockDetails = new StockDetailsForm(_marketType, stockName, reportType, industry);
            var frmStockDetails = new FinDetailsForm(_marketType, stockName, reportType, industry);
            frmStockDetails.MdiParent = this;
            frmStockDetails.Text = stockName + string.Format("-个股详细财务指标（市值={0}，PB={1}，PE-TTM={2},ROE={3}）", mc, pb, pettm, roe);
...
(case s.is_follow when 1 then '已关注' else '' end) 是否关注,(s.chg720) `涨跌幅720天`,(s.chg360) `涨跌幅360天`,
TIMESTAMPDIFF(year,s.issue_date,CURDATE()) 上市时间,convert_to_decimal(s.pe_ttm) PE,convert_to_decimal(s.pb) PB,m.jqjzcsyl ROE,
...
order by s.market_capital desc {1}";
                ...
                if (!string.IsNullOrEmpty(condition.Pb))
                {
                    if (condition.Pb.Contains(","))
                        where_addition += string.Format(" and s.pb>={0} and s.pb<={1} ", condition.Pb.Split(',')[0], condition.Pb.Split(',')[1]);
                    else
                ...
                if (condition.ReportType == "3")
                    ...
                    where_addition += " and m.date='2020-6-30'";
...
where m.date='2019-12-31' and s.tradetype='港股通' {0} order by s.market_capital desc {1}";
        ...
        {
            GenerateXYPositionForm form = new GenerateXYPositionForm();
            form.MdiParent = this;
            form.Text = "转换拿地所在城市经纬度坐标";
            form.Show();
