...
{
    public partial class BizModelGenerationForm : Form
    {
        ...
        public BizModelGenerationForm(string ts_code)
        {
            InitializeComponent();
...
");
        ...
        private void ShowPictureBoxTooltip()
        {
            ...
            foreach (Control child in children)
            {
                ...
                {
                    if (!(subChild is Panel))
                        continue;

                    var thirdChildren = subChild.Controls;
                    foreach (Control thirdChild in thirdChildren)
                    {
                        if (thirdChild is PictureBox)
                            thirdChild.DoubleClick += PictureBoxClick;

                        NestEachChildControls(thirdChild);
                    }
                }
            }
        }

        private void NestEachChildControls(Control thirdChild)
        {
            if (!(thirdChild is TableLayoutPanel))
                return;

            var fourthChildren = thirdChild.Controls;
            foreach (Control fourthChild in fourthChildren)
            {
                if (!(fourthChild is Panel))
                    continue;

                var fifthChildren = fourthChild.Controls;
                foreach (Control fifthChild in fifthChildren)
                {
                    if (fifthChild is PictureBox)
                        fifthChild.DoubleClick += PictureBoxClick;

                    if (fifthChild is TableLayoutPanel || fifthChild is Panel)
                    {
                        var sixthChildren = fifthChild.Controls;
                        foreach (Control sixthChild in sixthChildren)
                        {
                            if (sixthChild is PictureBox)
                                sixthChild.DoubleClick += PictureBoxClick;
                        }
                    }
                }
            }
